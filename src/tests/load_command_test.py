...
class LoadCommandTest(unittest.TestCase):
    ...
    FIXED_WIDTH_FILEPATH = 'D:/Projetos/milho-sqlite3/src/tests/test_content/teste.delimited'
    ...
    def test_load_command_with_truncate_true_and_table_not_exists(self):
        """Testa o processo de LOAD quando a tabela não existe no banco de dados e o parâmetro truncate é True
        É esperado que o processo de LOAD seja executado sem sucesso (erro) 
        """
        args = f'''{LoadCommandTest.ENTRYPOINT} {LoadCommandTest.COMMAND} '''
        args += f'''--from-file="{LoadCommandTest.EXCEL_FILEPATH}" '''
        args += f'''--file-type="excel" '''
